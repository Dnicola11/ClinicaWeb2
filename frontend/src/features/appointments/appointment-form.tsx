    ...
    <form onSubmit={handleSubmit(onSubmit)} className="h-full">
      <div className="flex flex-col justify-between h-full">
        <div className="flex flex-col space-y-3">
          <FormCalendar label="Tiempo de Inicio" name="start time" control={control} />
          <FormInputNumber label="Duración" name="duration" control={control} />
          <FormDropdown label="Paciente" name="client" control={control} options={clientOptions} optionLabel="nickname" />
          <FormDropdown label="Especialidad" name="service" control={control} options={serviceOptions} optionLabel="name" additionnalChange={handleServiceChange} />
          <FormDropdown label="Medico" name="expert" control={control} options={expertOptions} optionLabel="nickname" />
          <FormInputText label="Notas" name="notes" control={control} type="area" />
          <FormSelectButton label="Estado" name="status" control={control} options={STATUS_OPTIONS} />
        </div>
